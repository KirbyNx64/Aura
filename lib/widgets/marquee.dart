...
class IntegralCurve extends Curve {
  ...
  factory IntegralCurve(Curve original) {
    ...
    // Normalize.
    // Corregido: iterar sobre una copia de las claves para evitar modificar el mapa mientras se itera
    for (final double t in List<double>.from(values.keys)) {
...
class _MarqueeState extends State<Marquee> with SingleTickerProviderStateMixin {
  ...
  Future<void> _moveLinearly() async {
    ...
    if (fadeStartDuration > Duration.zero) {
      ...
      // Si la duración es muy corta, desvanecer inmediatamente
      setState(() => _isAboutToPause = true);
      await _animateTo(_linearTarget, linearDuration, Curves.linear);
    }
  }
