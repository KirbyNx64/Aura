...
class _SlidingUpPanelState extends State<SlidingUpPanel>
  ...
  void _onGestureSlide(double dy) {
    // if the panel is open and we are in the list area, we force scroll mode
    // to prevent accidental closing when swiping down at the top of the list.
    if (!_forceScrollingDisabled &&
        _isPanelOpen &&
        _sc.hasClients &&
        dy > 0 &&
        _sc.offset <= 0) {
    ...
    if (!_forceScrollingDisabled && _isPanelOpen && _sc.hasClients) {
      if (_sc.offset > 0) {
        if (!_scrollingEnabled) {
          setState(() {
            _scrollingEnabled = true;
        ...
        setState(() {
          if (dy < 0) {
            ...
            // Only allow switching to panel mode (to close) if forced (e.g. from header).
            // Since we are inside a !_forceScrollingDisabled block, we stay in scroll mode.
            _scrollingEnabled = true;
          }
        });
      }
  ...
  bool _forceScrollingDisabled = false;

  void _setScrollingEnabled(bool enabled) {
    setState(() {
      _scrollingEnabled = enabled;
      _forceScrollingDisabled = !enabled;
    });
  }
}
...
class PanelController {
  ...
  /// Sets whether the panel scrolling logic is enabled.
  /// Set to false to force panel dragging even if content is scrolled.
  void setScrollingEnabled(bool enabled) {
    assert(isAttached, "PanelController must be attached to a SlidingUpPanel");
    _panelState!._setScrollingEnabled(enabled);
  }
}
