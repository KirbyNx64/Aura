    ...
    if (showIndeterminateIndicator) {
      ...
      return _ContainedDragExpressiveIndicator(
        ...
        backgroundColor:
            (widget.backgroundColor ??
                    Theme.of(context).colorScheme.surfaceContainerHighest)
                .withValues(alpha: (_valueColor.value?.a ?? 0.0) / 2.0),
        progress: _value.value,
        polygons: widget.polygons,
        constraints: widget.indicatorConstraints,
        semanticsLabel: widget.semanticsLabel,
...
class _DragExpressiveIndicator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final activeMorphIndex = (morphSequence.length * progress).floor().clamp(
      0,
            ...
            child: Transform.rotate(
              ...
              child: CustomPaint(
                painter: _MorphPainter(
                  ...
                  color: color,
                  scaleFactor: 0.6, // Scale factor for drag state
                ),
                child: const SizedBox.expand(),
              ),
            ),
...
class _ContainedExpressiveLoadingIndicatorState
  ...
  Widget build(BuildContext context) {
    final constraints = widget.constraints ?? _defaultConstraints;
    final activeIndicatorScale =
        28.0 / math.min(constraints.maxWidth, constraints.maxHeight);
    final shapesScaleFactor =
        _calculateScaleFactor(_polygons) * activeIndicatorScale;
